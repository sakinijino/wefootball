class OfficalTeam < ActiveRecord::Base
  include ModelHelper
  
  DEFAULT_IMAGE = "/images/default_team_small.gif"
  
  has_one :offical_team_image,
            :dependent => :destroy
  ...
  def image(thumbnail = nil, refresh = nil)
    if refresh == :refresh && self.offical_team_image != nil
      self.image_path = self.offical_team_image.public_filename
      self.save
      return self.offical_team_image.public_filename(thumbnail) 
    end
