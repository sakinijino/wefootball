...
def netease_extract_matches(doc, year, month, day)
  ...
  rt = doc.css('table.daTb01 tr').map do |tr|
    ...
    match[:host] = tr.css("span.c1 a").first.content
    match[:guest] = tr.css("span.c2 a").first.content
    ...
    regmat = /(\d*)-(\d*)/.match(tr.css("span.c2 a").first.content)
    if regmat
      match[:host_goal] = reg[1].to_i
      match[:guest_goal] = reg[2].to_i
    end
    match
  end
